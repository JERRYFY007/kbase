...
def addlog(txt, fileName='FAQ_log.txt'):
...
class FAQrobot():
    ...
    def juziSim_simple(self, intxt, questionWordset):  # juziIn输入的句子，juziLi句子库里的句子
        ...
        for t in intxtSet:
            if t in questionWordset:
                simWeight += 1
        # print("simWeight:", simWeight, len(intxtSet))
    ...
    def juziSim_simple_POS(self, intxt, questionWordset, posWeight=None):  # juziIn输入的句子，juziLi句子库里的句子
        ...
        # print("simWeight, totalWeight:", simWeight, totalWeight)
        if totalWeight == 0:
            return 0
...
if __name__ == '__main__':
    ...
    robot = FAQrobot('FAQ_IT.txt', usedVec=True)
    while True:
        print('回复：' + robot.answer(input('输入：'), 'all') + '\n')  # simType=simple, simple_POS, vec, all
